---
title: 清理vps空间与chatgpt的一次聊天 | aoao.life
...
|  |  |
| --- | --- |
| ``` 1 2 3 ``` | ```  docker exec -it <容器名称或ID> du -sh / docker exec -it <容器名称或ID> df -h ``` |
...
|  |  |
| --- | --- |
| ``` 1 2 ``` | ```  docker container inspect <容器名称或ID> --format='{{.GraphDriver.Data.Usage}}' ``` |
...
|  |  |
| --- | --- |
| ``` 1 2 ``` | ```  git branch --merged | grep -v "\*" | xargs -n 1 git branch -d ``` |

要删除已经过期的标签，可以使用以下命令：

|  |  |
| --- | --- |
| ``` 1 2 ``` | ```  git tag --merged | xargs -n 1 git tag -d ``` |
