document.addEventListener('DOMContentLoaded', function() {
    ...
    if (postEl) { 
        POST_ID = postEl.getAttribute('data-post-id').replace(/\.html$/, ''); 
        initInteraction(); 
    }
    ...
    window.loadComments = async function() {
        ...
        try {
            ...
            data.comments.forEach(c => {
                ...
                li.style.cssText = "display: flex; gap: 15px; margin-bottom: 20px; border-bottom: 1px dashed #eee; padding-bottom: 15px;";
                li.innerHTML = `
                    <img src="${c.avatar}" style="width: 48px; height: 48px; border-radius: 50%; background: #eee;">
                    <div style="flex: 1;">
                        <div style="margin-bottom: 5px;"><strong style="color: #333;">${c.author}</strong> <span style="color: #999; font-size: 12px; margin-left: 10px;">${timeAgo(c.created_at)}</span></div>
                        <div style="color: #444; line-height: 1.6; word-break: break-all;">${c.content.replace(/\n/g, '<br>')}</div>
                    </div>`;
                ul.appendChild(li);
            });
        } catch (e) {
    ...
    window.submitComment = async function() {
        ...
        if (!author || !email || !content) return showToast("请填写完整信息哦！");

        btn.disabled = true;
        const originalText = btn.innerText;
        btn.innerText = "正在思考中...";  // 拟人化处理

        try {
            const res = await fetch(`${API_URL}/api/comment`, {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ post_id: POST_ID, author, email, content })
            });
